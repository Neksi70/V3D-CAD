...
> Hinweis: Persönliche Daten (genaue Anschrift, E-Mail, Azure-Kennungen) stehen bewusst
> NICHT in dieser Fassung. Cowork holt sie direkt beim Auftraggeber ab.
...
Beim Auftraggeber erfragen: vollständiger Name, Privatanschrift, E-Mail-Adresse für das
Certum-Konto. (Firmendaten werden für dieses Produkt NICHT gebraucht — es läuft auf
die Privatperson.)
...
3. Certum-Konto wird angelegt; Aktivierungs-Mail kommt an die hinterlegte Adresse.
...
6. EXE signieren (auf dem Windows-Rechner, SimplySign Desktop muss verbunden sein):
   ...
   ```
   signtool sign /n "<Name laut Zertifikat>" /fd sha256 /tr http://time.certum.pl /td sha256 V3D-Remote.exe
...
8. Azure aufräumen, damit dort keine Kosten mehr auflaufen: im Azure-Portal den
   Artifact-Signing-Account `v3dremote-signing` und die Ressourcengruppe `v3d-signing` löschen
   und die laufende Identitätsprüfung zurückziehen. Zugangs-/Subscription-Daten beim
   Auftraggeber erfragen, vorher seine Freigabe holen.

## Wichtig zu wissen (bitte dem Auftraggeber so sagen)

- Die Signatur beseitigt „Unbekannter Herausgeber" — der Name des Auftraggebers steht dann
  als Herausgeber in der Windows-Abfrage.
